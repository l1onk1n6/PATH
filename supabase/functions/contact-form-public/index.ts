...
/** SHA-256 hex of a string — used to hash IPs before storing */
...
Deno.serve(async (req) => {
  ...
  if (!email?.trim()) {
    return json({ error: 'E-Mail-Adresse ist erforderlich.', step: 'validation' }, 400)
  }
  ...
  if (!zammadUrl || !zammadToken) {
    ...
    return json({ error: 'Ticket-System nicht konfiguriert. Bitte direkt an info@pixmatic.ch schreiben.', step: 'zammad' }, 500)
  }
  ...
  try {
    ...
    return json({ error: `Zammad nicht erreichbar: ${String(fetchErr)}`, step: 'zammad' }, 500)
  }

  if (!zammadRes.ok) {
    const errText = await zammadRes.text().catch(() => '')
    console.error('Zammad API error:', zammadRes.status, errText)
    return json({ error: `Ticket konnte nicht erstellt werden (${zammadRes.status}).`, step: 'zammad' }, 500)
  }
...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
  ...
  _step = 'log'
  const { error: logErr } = await admin.from('contact_log_public').insert({ ip_hash: ipHash })
